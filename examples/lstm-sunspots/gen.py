...
cwd = os.getcwd()
if os.path.basename(cwd) == 'lstm-sunspots':
    save_dir = cwd
else:
    save_dir = os.path.join(cwd, 'examples', 'lstm-sunspots')

# Downloading the dataset.
if not os.path.exists(os.path.join(save_dir, 'Sunspots.csv')):
...
df = pd.read_csv(os.path.join(save_dir, 'Sunspots.csv'))
...
model = tf.keras.Sequential([
    tf.keras.layers.LSTM(100, return_sequences=True, name='lstm1'),
    tf.keras.layers.LSTM(100, name='lstm2'),
